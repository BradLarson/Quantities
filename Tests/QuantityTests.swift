...
class QuantityTests: XCTestCase {
    ...
    func testArithmetic() {
        ...
        XCTAssertEqual(distance1.value(in:.centimeters), 401, accuracy:0.001)

        let distance2 = Distance(4.0, unit:.meters) - Distance(1.0, unit:.centimeters)
        XCTAssertEqual(distance2.value(in:.centimeters), 399, accuracy:0.001)

        let distance3 = Distance(0.0, unit:.meters) - Distance(1.0, unit:.centimeters)
        XCTAssertEqual(distance3.value(in:.centimeters), -1, accuracy:0.001)
        
        let distance4 = Distance(4.0, unit:.meters) * 2.0
        XCTAssertEqual(distance4.value(in:.centimeters), 800, accuracy:0.001)

        let distance5 = 2.0 * Distance(4.0, unit:.meters)
        XCTAssertEqual(distance5.value(in:.centimeters), 800, accuracy:0.001)

        let distance6 = Distance(4.0, unit:.meters) / 2.0
        XCTAssertEqual(distance6.value(in:.centimeters), 200, accuracy:0.001)

        let distance7 = Distance(4.0, unit:.meters) / Distance(2.0, unit:.meters)
        XCTAssertEqual(distance7, 2.0, accuracy:0.001)

        let distance8 = -Distance(4.0, unit:.meters)
        XCTAssertEqual(distance8.value(in:.centimeters), -400, accuracy:0.001)

        let distance9 = abs(-Distance(4.0, unit:.meters))
        XCTAssertEqual(distance9.value(in:.centimeters), 400, accuracy:0.001)
