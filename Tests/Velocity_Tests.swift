...
class Velocity_Tests: XCTestCase {
    ...
    func testConversion() {
        ...
        XCTAssertEqualWithAccuracy(velocity1.value(in:.metersPerSecond), 0.44704, accuracy:0.0001)
        XCTAssertEqualWithAccuracy(velocity1.value(in:.milesPerHour), 1.0, accuracy:0.001)
        XCTAssertEqualWithAccuracy(velocity1.value(in:.kilometersPerHour), 1.609344, accuracy:0.00001)

        let velocity2 = Velocity(1.0, unit:VelocityUnit(numerator:.meters, denominator:.seconds))
        XCTAssertEqualWithAccuracy(velocity2.value(in:.metersPerSecond), 1.0, accuracy:0.0001)
    ...
    func testArithmetic() {
        ...
        XCTAssertEqualWithAccuracy(velocity1.value(in:.metersPerSecond), 2.0, accuracy:0.0001)

        let distance1 = Velocity(1.0, unit:.metersPerSecond) * Time(3.5, unit:.seconds)
        XCTAssertEqualWithAccuracy(distance1.value(in:.meters), 3.5, accuracy:0.0001)
    }
